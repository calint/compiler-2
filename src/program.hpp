...
public:
  inline explicit program(const std::string &source) : tc_{source} {
    ...
    while (true) {
      ...
      if (tk.is_empty()) {
        ...
        throw compiler_exception(tk, "unexpected '" +
                                         std::string{tkz.next_char()} + "'");
      }
      if (tk.is_name("field")) {
        statements_.emplace_back(
            std::make_unique<stmt_def_field>(tc_, tk, tkz));
      } else if (tk.is_name("func")) {
        statements_.emplace_back(std::make_unique<stmt_def_func>(tc_, tk, tkz));
      } else if (tk.is_name("type")) {
        statements_.emplace_back(std::make_unique<stmt_def_type>(tc_, tk, tkz));
      } else if (tk.name().starts_with("#")) {
        statements_.emplace_back(std::make_unique<stmt_comment>(tc_, tk, tkz));
      } else if (tk.is_name("")) {
        //? should this be able to happen?
        // throw panic_exception("program:1");
        statements_.emplace_back(std::make_unique<statement>(tk, unary_ops{}));
      } else {
  ...
  inline void compile(toc &tc, std::ostream &os, size_t indent,
    ...
    os << std::endl;
    os << "; system call: exit 0" << std::endl;
    os << "mov rax, 60" << std::endl;
    os << "mov rdi, 0" << std::endl;
    os << "syscall" << std::endl;
  }

  inline void build(std::ostream &os) {
    compile(tc_, os, 0);
    tc_.finish(os);
  }
};
