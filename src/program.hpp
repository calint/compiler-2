...
class program final:public statement{public:
	inline program(tokenizer&t):statement{nullptr,make_unique<class token>()}{
		while(!t.is_eos()){
			...
			else{stmt=create_statement(tk->name(),this,move(tk),t);}
			statements.push_back(move(stmt));
		}
	}
	inline void build(ostream&os){compile(tc,os);link(tc,os);}
	inline void compile(toc&tc,ostream&os)override{os<<"section .text\nglobal _start\n_start:\n";for(auto&s:statements)s->compile(tc,os);}
	inline void link(toc&tc,ostream&os)override{for(auto&s:statements)s->link(tc,os);}
	inline void source_to(ostream&os)override{statement::source_to(os);for(auto&s:statements)s->source_to(os);}
