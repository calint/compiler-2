...
class program final {
  ...
  public:
    ...
    auto compile(toc& tc, x86& x, const size_t indent) const -> void {
        size_t total_dat_size_bytes{};
        for (const statement* stmt : tc.get_data()) {
            total_dat_size_bytes += stmt->dat_size_bytes();
        }

        const size_t dat_padding{
            (stack_and_data_alignment_ -
             (total_dat_size_bytes % stack_and_data_alignment_)) %
            stack_and_data_alignment_};

        if (dat_padding != 0) {
            const token& src_loc_tk{
                tc.get_data().empty() ? token{} : tc.get_data().back()->tok()};
            throw compiler_exception{
                src_loc_tk,
                std::format(
                    "total dat size {} B is not aligned to {} B; add "
                    "{} B of dat padding, e.g.: dat dat_padding : i8[{}]",
                    total_dat_size_bytes, stack_and_data_alignment_,
                    dat_padding, dat_padding)};
        }
