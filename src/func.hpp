...
class func:public statement{public:
	func(statement*parent,up_token tkn,tokenizer&t):statement{parent,move(tkn)}{
		...
		statements=make_unique<func_statements>(parent,t);
	}
	void compile(toc&tc)override{tc.put_func(identifier->name());}
	void link(toc&tc)override final{statements->link(tc);}
	...
	void source_to_stdout()override{
		...
		printf(")");
		statements->source_to_stdout();
	}

private:
	up_token identifier;
	vup_tokens params;
	up_func_statements statements;
};
