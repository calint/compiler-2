...
public:
	...
	inline void compile(toc&tc,ostream&os,size_t indent,const string&dst="")const override{
		os<<"; generated by baz\n\n";
		// built-in type
		const type&type_qword{"qword",8,true};
		const type&type_dword{"dword",4,true};
		const type&type_word {"word" ,2,true};
		const type&type_byte {"byte" ,1,true};
		tc.add_type(*this,type_qword);
		tc.add_type(*this,type_dword);
		tc.add_type(*this,type_word);
		tc.add_type(*this,type_byte);
