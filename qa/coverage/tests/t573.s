...
main:
    ...
    # [6:9] k = -1
    # [6:14] -1
    # [6:14] allocate scratch register -> t0
    li t0, -1
    sw t0, 8(s0)
    # [6:14] free scratch register t0
    # [7:5] var n = 1
    # [7:9] n: i32 (4 B @ [s0 + 12])
    # [7:9] n = 1
    # [7:13] 1
    # [7:13] allocate scratch register -> t0
    li t0, 1
    sw t0, 12(s0)
    # [7:13] free scratch register t0
    # [8:5] var r = read(0, buf, n, k)
    # [8:9] r: i32 (4 B @ [s0 + 16])
    # [8:9] r = read(0, buf, n, k)
    # [8:13] r = read(0, buf, n, k)
    # [8:13] = expression
    # [8:13] read(0, buf, n, k)
    ...
    # [8:26] n
    lw a2, 12(s0)
    # [8:29] allocate scratch register -> t0
    # [8:29] k
    lw t0, 8(s0)
    # [8:29] bounds check
    bltz t0, 1f
    # [8:29] allocate scratch register -> t1
    # [8:29] allocate scratch register -> t2
    # [8:29] allocate scratch register -> t3
    srai t3, t0, 31
    srai t1, a2, 31
    add t3, t3, t1
    add t2, t0, a2
    ...
    j baz_bounds_panic
    2:
    # [8:29] free scratch register t3
    # [8:29] free scratch register t2
    # [8:29] free scratch register t1
    addi a1, s0, 0
    add a1, a1, t0
