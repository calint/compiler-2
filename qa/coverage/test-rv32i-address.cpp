...
auto main(const int argc, const char* argv[]) -> int {
    ...
    // 'a0, a1' as 'a0' and 'a1'
    const auto split_operands =
        [](const std::string_view operands) -> std::vector<std::string_view> {
    ...
    {
        // 'name:' lines are labels, '# ' lines comments, 'j' and branches
        // jump, and 'addi', 'ecall' and 'call' are instructions
        const auto optimize =
...
func main() {
        ...
        for (size_t size_bytes{}; size_bytes <= 24; ++size_bytes) {
            for (size_t alignment{}; alignment < 4; ++alignment) {
                ...
                for (size_t offset{}; offset < 32; ++offset) {
                    ...
                    const size_t expected{offset >= start and
    ...
    for (const size_t scale :
        ...
        std::println("    li a3, {}\n    beq a0, a3, 1f\n    j failure\n1:",
                     std::bit_cast<int32_t>(uint32_t{0xfffffff9} *
